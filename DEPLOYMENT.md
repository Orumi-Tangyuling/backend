...
- 앱 배포: GitHub Actions에서 SSH로 서버 접속 후 `docker compose` 갱신
...
- 동작:
  ...
  3. 서버 `~/tangyuling`에 `docker-compose.prod.yml`, `.env` 반영
...
3. 앱 기동 실패
   - `ENV_FILE` 값 확인 (`DATABASE_URL`, `MYSQL_ROOT_PASSWORD` 등)
